(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '@media(max-width:680px){body.astro-consent-open .hero{padding-top:calc(var(--nav-h,72px) + var(--astro-consent-space,0px) + 24px)}body.astro-consent-open .home-v2-page .home-hero{padding-top:calc(var(--nav-h,72px) + var(--astro-consent-space,0px) + 18px + env(safe-area-inset-top))}.astro-consent{box-sizing:border-box;display:grid;top:calc(var(--nav-h,72px) + 10px + env(safe-area-inset-top));bottom:auto;left:10px;right:10px;gap:12px;max-width:calc(100vw - 20px);max-height:min(46vh,320px);overflow-y:auto;padding:12px}.astro-consent__text{gap:5px;font-size:12.5px;line-height:1.45}.astro-consent__actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.astro-consent__button{min-height:44px;width:100%;padding:10px 12px}}'
    ].join('');
