(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      '.astro-consent{position:fixed;left:16px;right:16px;bottom:max(8px,env(safe-area-inset-bottom));z-index:9999;display:flex;gap:14px;align-items:center;justify-content:space-between;max-width:980px;margin:auto;padding:12px;border:1px solid rgba(255,255,255,.16);border-radius:6px;background:rgba(15,15,14,.96);box-shadow:0 18px 60px rgba(0,0,0,.35);color:#fff;font-family:Inter,Arial,sans-serif}',
      ...
      '.astro-consent__actions{display:flex;gap:10px;flex-shrink:0}.astro-consent__button{min-height:44px;border:1px solid #dfc9a6;background:#dfc9a6;color:#101010;border-radius:4px;padding:8px 14px;font-weight:700;cursor:pointer}.astro-consent__button--ghost{background:transparent;color:#fff;border-color:rgba(255,255,255,.3)}',
      ...
      '@media(max-width:680px){.astro-consent{box-sizing:border-box;display:grid;bottom:max(8px,env(safe-area-inset-bottom));left:max(8px,env(safe-area-inset-left));right:max(8px,env(safe-area-inset-right));gap:8px;max-width:calc(100vw - 16px - env(safe-area-inset-left) - env(safe-area-inset-right));max-height:min(32vh,210px);overflow-y:auto;padding:10px}.astro-consent__text{display:block;font-size:12px;line-height:1.35}.astro-consent__text strong{display:inline;margin-right:6px;font-size:13px}.astro-consent__text span{display:inline}.astro-consent__text a{display:inline-block;margin-top:3px}.astro-consent__actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.astro-consent__button{min-height:44px;width:100%;padding:8px 10px}}'
    ].join('');
